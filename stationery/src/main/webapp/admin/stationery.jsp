...
<div class="box-header with-border">
    <h3 class="box-title">文具领用</h3>
</div>
<div class="box-body">
    <!-- 管理员权限 -->
    <c:if test="${USER_SESSION.role =='ADMIN'}">
        <button class="btn btn-primary" onclick="onAddStationeryBtn()">新增领用</button>
    </c:if>
    <!--工具栏 数据搜索 -->
    <div class="box-tools pull-right">
        <div class="has-feedback">
            <form action="${pageContext.request.contextPath}/stationeryRequisition/search" method="post">
            ...
            <c:forEach items="${pageResult.rows}" var="rows">
                <tr>
                    ...
                    <td class="text-center">
                        <c:if test="${rows.status ==0}">
                            <button type="button" class="btn bg-olive btn-xs" data-toggle="modal"
                                    data-target="#borrowModal" onclick="findStationeryById(${rows.id},'borrow')"> 领用
                            </button>
                            <c:if test="${USER_SESSION.role =='ADMIN'}">
                                <button type="button" class="btn bg-olive btn-xs" data-toggle="modal"
                                        data-target="#addOrEditModal2" onclick="findStationeryById(${rows.id},'edit')"> 编辑
                                </button>
                            </c:if>
                        </c:if>
                        <c:if test="${rows.status ==1 ||rows.status ==2}">
                            <button type="button" class="btn bg-olive btn-xs" disabled="true">领用</button>
                        </c:if>
                    </td>
                </tr>
            </c:forEach>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addOrEditForm">
                    <span><input type="hidden" id="aebid" name="id"></span>
